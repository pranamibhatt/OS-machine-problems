...
VMPool::VMPool(unsigned long _base_address, unsigned long _size, FramePool* _frame_pool, PageTable* _page_table)
{
	...
	capacity = PG_SIZE / (sizeof(vmpool));
	pool = (vmpool *)(frame_pool->get_frame_address(frame_pool->get_frame()));	
	//pool = (vmpool *)(frame_pool->get_frame());
//	memset(pool, 0, PG_SIZE);
...
void VMPool::release(unsigned long _start_address)
{
	...
	for (int j = 0; j < pool[i].size/PG_SIZE; j++)
	{
		page_table->free_page(_start_address);
		_start_address = _start_address + PG_SIZE;
	}
