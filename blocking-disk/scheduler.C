...
void Scheduler :: yield ()
{
	...
	// if the disk is ready, get the thread from the disk
	// and dispatch it
	extern SimpleDisk *SYSTEM_DISK;	
	extern BlockingDisk *DISK;	
	Thread *ret_thread = SYSTEM_DISK->get_disk_thread();
	// if the disk is not ready,get the thread from the ready Q
	if (ret_thread == NULL)
	{
